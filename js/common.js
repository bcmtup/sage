window.addEventListener('DOMContentLoaded', function() {
    ...
    $('.alert-socket__btn').click(function() {
        ...
        $('.alert-socket__content').toggle(500, function() {
            ...
            let time = $(this).find('li:first-child').data('notification_time');
            if (time) {
                cookie('messageIteration', time, {
                    expires: 2592e6
                });
            }
        });
...
function eventsWebSocket() {
    ...
    // var socket = io.connect('ws://eth.forsage.loc:2087', {
    ...
    function writeByContainer(events) {
        ...
        for (let i = 0; i < events.length; i++) {
            ...
            if (((event.matrix == '1' || event.matrix == '2') && params.storage.notify.lastKeyXBase < event.id) || (event.matrix == '3' && params.storage.notify.lastKeyXGold < event.id)) {
                // Добавить запись
                params.container.el.prepend(`<li data-notification_time="${event.timestamp}"><img src="/img/icons/${params.eventsTypeIcon[event.eventName]}" alt="" class="alert-socket__img"> ${msg} <div data-elapse_time="${event.time}" class="elapsedTime"></div></li>`);
                ...
                for (let i = params.container.el.find('li').length; i > params.container.countItems; i--) {
                    params.container.el.find('li:last-child').remove();
                }
    ...
    function createMessage(event) {
        ...
        if (event.matrix > 0) {
            ...
            msg = msg.replace('{currency_usd}', parseFloat(config.course.ETH_USD * event.level_price).toFixed(2));
        }
        switch (event.matrix) {
            case '1':
        ...
        msg += '<a href="https://etherscan.io/tx/' + event.tx + '" target="_blank" class="notifications-link__hash"><i class="fas fa-external-link-alt"></i></a>';
        return msg;
    }
    ...
    function messageIteration(action, key) {
        ...
        if (action == 'set') {
            ...
            cookie('messageIteration', params.messageIteration, {
                expires: 2592e6
            });
        }
    }
...
(function() {
    ...
    if ($('.matrix__toggle_btn')) {
        ...
        $('.matrix__header').on('click', function() {
    ...
    window.Notice = new Vue({
        ...
        methods: {
            show(text, params) {
                ...
                if (item.lifetime > 0) item._timeout = setTimeout(() => {
                    this.close(item);
                }, item.lifetime);
            },
            close(item) {
...
function hook(e) {
        ...
        el.startY = (e.type == 'mousedown') ? (e.clientY) : 0;
...
function move(e) {
        ...
        if(el.startY)
            el.style.height = el.offsetHeight - (e.clientY - el.startY) + 'px';
